...
mod_firstSeenBeat = GAMESTATE:GetSongBeatVisible();
		...
		if mbeat > 92 and mbeat < 109 then
			for i=1,#dm_questiontiming do
				...
				if mbeat-dm_questiontiming[i] < 0.25 and mbeat-dm_questiontiming[i] > -0.1 then
					dm_spawnf(params.Col,pn)
					for c=1,#dm_brain do
						if dm_brain[c]:getaux() == params.Col+1 and (pn == 1 and dm_brain[c]:GetX() < sw/2 or pn == 2 and dm_brain[c]:GetX() > sw/2) then
							dm_brain[c]:stoptweening();
							dm_brain[c]:queuecommand('Hide');
						end
					end
					dm_myanswer[pn] = _G['dm_quiz_choices'..pn][i][params.Col+1]
...
t[#t+1] = Def.ActorFrame{
	...
	LoadActor('brainshower')..{
		...
		StepMessageCommand=function(self,params)
			pn = 2
			if params.PlayerNumber == 'PlayerNumber_P'..pn and GAMESTATE:GetSongBeatVisible() > 282 and GAMESTATE:GetSongBeatVisible() < 308 then
...
function mawaru_horse_update()
	local b = GAMESTATE:GetSongBeatVisible()
...
t[#t+1] = LoadActor(THEME:GetPathS("","EvalAnnouncer/RANK_A"))..{
...
t[#t+1] = Def.ActorFrame{
	Def.ActorFrame{
		...
		Def.Quad{
			OnCommand=cmd(visible,false);
			StepMessageCommand=function(self,params)
				if GAMESTATE:GetSongBeatVisible() > 418 and GAMESTATE:GetSongBeatVisible() < 446 then
					for pn=1,2 do
...
t[#t+1] = Def.ActorFrame{
	...
	Def.Quad{
		StepMessageCommand=function(self,params)
			for pn=1,2 do
				if params.PlayerNumber == 'PlayerNumber_P'..pn and params.Col == 2 then
					botw_held[pn] = true;
				end
			end
		end,
	...
	Def.ActorFrame{
		...
		LoadActor("botw_slash")..{
			...
			OnCommand=cmd(valign,0.9;diffusealpha,0;blend,Blend.Add;animate,false);
		},
...
t[#t+1] = Def.ActorFrame{
	...
	Def.Quad{
		...
		StepMessageCommand=function(self,params)
			for pn=1,2 do
				if params.Col == 2 and params.PlayerNumber == 'PlayerNumber_P'..pn then
					if tower_curpiece[pn] > 0 and GAMESTATE:GetSongBeatVisible() > 464 and GAMESTATE:GetSongBeatVisible() < 490 then
						local a = _G['tower_p'..pn..'brick'..tower_curpiece[pn] ]
...
t[#t+1] = LoadActor("0")..{
	...
	LiftMessageCommand=function (self, params)
		--SCREENMAN:SystemMessage("Lift "..params.Col);
	end;
}
...
t[#t+1] = Def.Sprite{
	...
	JudgePlayersMessageCommand=function(self)
		...
		if GAMESTATE:GetSongBeatVisible() < 650 then
			if allfailed then
				Sound('wrong.ogg')
			else
				Sound('good.ogg')
...
function mawaru_closedoor(bpm)
	...
	if GAMESTATE:GetSongBeatVisible() > 50 then
...
t[#t+1] = LoadActor("0")..{
	...
	UpdateCommand=function (self)
		...
		local beat = GAMESTATE:GetSongBeatVisible()
		local mbeat = ((GAMESTATE:GetCurMusicSeconds()+5.274)/(60/128))
		...
		if beat > mod_firstSeenBeat+0.1 then -- performance coding!! --
			while curaction<=table.getn(actions) and GAMESTATE:GetSongBeatVisible()>=actions[curaction][1] do
				if actions[curaction][3] or GAMESTATE:GetSongBeatVisible() < actions[curaction][1]+2 then
