...
if GAMESTATE:IsSideJoined(PLAYER_1) == true and GAMESTATE:IsSideJoined(PLAYER_2) == true then
	...
	if steps1 == 'S22 MAXIMUS UCS' and steps2 == 'S22 MAXIMUS UCS' then
		single = true;
		player = 3;
	end;
	if steps1 == 'S22 MAXIMUS UCS' and steps2 ~= 'S22 MAXIMUS UCS' then
	...
	if GAMESTATE:IsSideJoined(PLAYER_1) then
		...
		if steps:GetStepsType() == 'StepsType_Pump_Single' and steps:GetDifficulty() == 'Difficulty_Edit' and steps:GetDescription() == 'S22 MAXIMUS UCS' then
			single = true;
			player = 1;
		end;
	end;
