...
if GAMESTATE:IsSideJoined(PLAYER_1) == true and GAMESTATE:IsSideJoined(PLAYER_2) == true then
	local steps1 = GAMESTATE:GetCurrentSteps(PLAYER_1):GetAuthorCredit();
	local steps2 = GAMESTATE:GetCurrentSteps(PLAYER_2):GetAuthorCredit();
	if steps1 == 'DulKi' and steps2 == 'DulKi' then
		single = true;
		player = 3;
	end;
	if steps1 == 'DulKi' and steps2 ~= 'DulKi' then
	...
	if GAMESTATE:IsSideJoined(PLAYER_1) then
		...
		if steps:GetStepsType() == 'StepsType_Pump_Single' and steps:GetDifficulty() == 'Difficulty_Edit' and steps:GetAuthorCredit() == 'DulKi' then
			single = true;
			player = 1;
		end;
	end;
